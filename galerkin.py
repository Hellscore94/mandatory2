...
class FunctionSpace:
    ...
    def eval_derivative_basis_function_all(self, Xj, k=1):
        D = np.zeros((len(Xj), self.N + 1))
        for j in range(self.N + 1):
            D[:, j] = self.evaluate_derivative_basis_function(Xj, j, k)
        return D
...
class Legendre(FunctionSpace):
    ...
    def L2_norm_sq(self, N):
        def squares(i):
            return 2/(2*i + 1)
        return np.array([squares(j) for j in range(N+1)])

    def mass_matrix(self):
        mass = sparse.diags([self.L2_norm_sq(self.N+1)], [0],
    ...
    def L2_norm_sq(self, N):
        def c(i):
            if i > 0:
                ...
                return 2
        return np.array([c(j)*np.pi/2 for j in range(N+1)])
    ...
    def L2_norm_sq(self, N):
        def c(i):
            if i > 0:
                ...
                return 1
        return np.array([c(j) for j in range(N+1)])
...
class NeumannLegendre(Composite, Legendre):
    def __init__(self, N, domain=(-1, 1), bc=(0, 0), constraint=0):
        Legendre.__init__(self, N, domain=domain)
        self.B = Neumann(bc, domain, self.reference_domain)
        self.S = sparse.diags(np.ones(1), 0, shape=(N+1, N+3), format='lil')
